...
#define TWITTER 1
#define NUM_DOCUMENTS 5712169
#define WORKING_SET    100000
...
namespace {
...
#if TWITTER
    const char* _db = "twitter";
    const char* _coll = "tweets";
#else
    ...
    int getRandomNumber(unsigned int *seedp) {
#ifdef RANDOMIZE_EVERY_SINGLE_CALL
	std::string threadId = boost::lexical_cast<std::string>(boost::this_thread::get_id());
	unsigned long threadNumber = 0;
	sscanf(threadId.c_str(), "%lx", &threadNumber);

	static ptime const epoch(date(1970, 1, 1));
	// this randomizes every single call
	srand(rand() + threadNumber + (boost::posix_time::microsec_clock::universal_time() - epoch).total_milliseconds());
#endif
	// see http://c-faq.com/lib/randrange.html
	int random_num = (int)((double)rand_r(seedp) / ((double)RAND_MAX + 1) * WORKING_SET);
	return random_num * (NUM_DOCUMENTS / WORKING_SET); // spread working set over the entire record space
    }
    ...
    void clearDB(){
#if TWITTER
	return; /* I never want to do this */
#endif
...
namespace Twitter {
	// run() takes threadId and totalThreads as arguments
	// iterations is global and is passed in on command line
	...
	struct RandomUpdates {
		...
		void run(int threadId, int totalThreads) {
			...
			for (int i = 0; i < iterations/totalThreads; i++) {
				int random_number = getRandomNumber(&seed);
                        	//cout << "Thread " << threadId << ": my random number is " << random_number << endl;
				update(threadId, BSON("_id" << random_number), BSON("$inc" << BSON("favorite_count" << 1)) );
			}
		}
	};

        struct RandomQueriesById {
                bool readOnly() { return true; }
                void reset() { /* do nothing */ }
                void run(int threadId, int totalThreads) {
			unsigned int seed = (unsigned int) threadId;
                        for (int i = 0; i < iterations/totalThreads; i++) {
                                int random_number = getRandomNumber(&seed);
                                //cout << "Thread " << threadId << ": my random number is " << random_number << endl;
                                findOne(threadId, BSON("_id" << random_number), BSON("favorite_count" << 1) );
                        }
                }
        };
}
...
#endif
#else
	    add< Twitter::RandomQueriesById >();
	    add< Twitter::RandomUpdates >();
#endif
